#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Clean Balkes Skor data to senior/professional-team records only.

v3.3 is intentionally target-focused:
- remove youth/PAF/academy/BAL/women/futsal contamination;
- suppress obviously partial or impossible seasons instead of publishing fake data;
- rebuild manifest/search/opponents/data_report from the cleaned data;
- optionally clear stale standings when match data changed.
"""
...
FACTORY_VERSION = "v3.3-targeted-professional-cleaner"
...
SIGNALS = (
    "profesyonel takim", "profesyonel takım", "super lig", "süper lig", "spor toto super lig",
    "spor toto süper lig", "1 lig", "1.lig", "1. lig", "1lig", "2 lig", "2.lig", "2. lig",
    "2lig", "3 lig", "3.lig", "3. lig", "3lig", "turkiye kupasi", "türkiye kupası",
    "ziraat turkiye kupasi", "ziraat türkiye kupası", "play off musabakalari",
    "play-off müsabakaları", "play off müsabakaları",
)
...
def is_senior_professional_competition(value: Any) -> bool:
    ...
    if any(norm(x) in n for x in BANNED):
        return False
    return any(norm(x) in n for x in SIGNALS)
...
def valid_match(m: dict[str, Any]) -> tuple[bool, str]:
    comp = m.get("competition") or m.get("competitionLabel") or ""
...
def read_detail_for_item(data_root: Path, item: dict[str, Any]) -> tuple[dict[str, Any], Path | None]:
    detail_url = str(item.get("detailUrl") or "")
    if detail_url:
        candidate = data_root / detail_url
        if candidate.exists():
...
def process_season_dir(season_dir: Path, args: argparse.Namespace) -> dict[str, Any]:
    ...
    rejected = Counter()
    removed_detail_paths: list[Path] = []

    for item in index:
        if not isinstance(item, dict):
            rejected["bad_index_item"] += 1
            continue
        detail, detail_path = read_detail_for_item(args.data_root, item)
        ok, reason = valid_match(detail)
        if not ok:
            rejected[reason] += 1
            if detail_path:
                removed_detail_paths.append(detail_path)
            continue
        kept.append(update_balkes_fields(detail))

    # Drop duplicates by real match signature, not by macId, because old TFF rows can expose the same fixture through multiple pages.
    seen = set()
    deduped = []
    ...
    changed = len(index) != len(kept) or bool(rejected)
    report: dict[str, Any] = {
        "season": season,
        "before": len(index),
        "after": len(kept),
    ...
    if args.write:
        ...
        for m in kept:
            ...
            new_index.append({k: m.get(k) for k in [
                "id", "matchCode", "season", "competition", "competitionType", "competitionLabel",
                "stadium", "venue", "stage", "stageLabel", "date", "time", "dateDisplay",
                "homeTeam", "awayTeam", "matchType", "matchTypeLabel", "type", "typeLabel",
                "score", "balkes", "quality", "detailCompleteness", "source", "detailUrl"
            ] if k in m})
        for p in matches_dir.glob("*.json"):
            if p.stem not in new_ids:
                p.unlink()
        ...
        if changed and not args.keep_standings_on_match_change:
            write_json(standings_path, [])
            report["standingsCleared"] = True
        elif not standings_path.exists():
            write_json(standings_path, [])
    return report
...
def rebuild_global(data_root: Path, reports_root: Path, reports: list[dict[str, Any]]) -> None:
    ...
    search = []
    seasons_root = data_root / "seasons"
    if seasons_root.exists():
        season_dirs = sorted([p for p in seasons_root.iterdir() if p.is_dir()], key=lambda p: p.name, reverse=True)
    else:
        season_dirs = []
    for season_dir in season_dirs:
        p = season_dir / "matches_index.json"
        arr = read_json(p, []) if p.exists() else []
        if not isinstance(arr, list) or not arr:
            continue
        season_json = read_json(season_dir / "season.json", {}) or {}
        item = {"id": season_dir.name, "name": season_dir.name, "matchCount": len(arr)}
        ...
        standings = season_dir / "standings_by_week.json"
        if standings.exists(): item["standingsByWeekUrl"] = f"seasons/{season_dir.name}/standings_by_week.json"
        seasons.append(item)
        for m in arr:
            b = m.get("balkes") or {}
            opp = b.get("opponent")
    ...
    if not (data_root / "players_index.json").exists():
    ...
    write_json(data_root / "data_report.json", {
        ...
        "notes": [
            "v3.3: youth/PAF/academy/BAL/women contamination removed.",
            "Partial seasons are suppressed instead of publishing misleading data.",
            "Standings are cleared for seasons whose match set changed, then rebuilt by the standings builder when enough full-league data exists."
        ],
    })
    reports_root.mkdir(parents=True, exist_ok=True)
    write_json(reports_root / "professional_guard_v33_summary.json", {"generatedAt": now(), "reports": reports})
...
def main() -> int:
    ...
    ap.add_argument("--keep-standings-on-match-change", action="store_true")
    ...
    print(f"v3.3 professional guard: seasons={len(reports)} changed={len(changed)} suppressed={len(bad)}")
    return 0
