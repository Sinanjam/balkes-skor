...
def load_dispatch_payload() -> dict[str, Any]:
    ...
    # v3.6 fix: GitHub repository_dispatch allows max 10 top-level
    # client_payload properties. We keep all chain settings inside one
    # nested "config" object and read it here.
    if isinstance(payload.get("config"), dict):
        cfg = dict(payload["config"])
        meta = payload.get("meta")
        if isinstance(meta, dict):
            cfg.update({f"meta_{k}": v for k, v in meta.items()})
        return cfg

    # Backward compatibility for older direct payloads.
    return payload
