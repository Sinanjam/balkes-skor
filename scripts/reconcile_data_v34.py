...
VERSION = "v3.4-summary-standings-reconcile"
...
def normalize_table_row(row: dict[str, Any], league_stats: dict[str, Any]) -> tuple[dict[str, Any], list[str]]:
    ...
    if result_points > table_points:
        ...
        if int(table.get("pointsDeducted") or 0) < inferred:
            ...
            table["penaltyNote"] = f"v3.4 inferred from official table: {inferred} point deduction"
...
def main() -> int:
    ...
    report = {
        ...
        "seasons": seasons,
        "notes": [
            "v3.4 keeps match-derived season stats separate from official league-table stats.",
    ...
    write_json(args.reports_root / "reconcile_v34_summary.json", report, args.write)
